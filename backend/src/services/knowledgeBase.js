...
export async function processQuery(
  ...
  try {
    ...
    if (multipleQuestions.isMultiple) {
      console.log(
        `[KB] Detected ${multipleQuestions.questions.length} questions`
      );
...
async function handleMultipleQuestions(
  ...
  console.log(`[KB] Processing ${questions.length} questions separately`);
  ...
  if (knownAnswers.length === 0) {
    // All questions unknown
    console.log("[KB] All questions unknown - escalating");
    return await handleUnknownQuery(originalQuery, participantId, roomName);
  }

  if (unknownQuestions.length > 0) {
    // Some known, some unknown
    console.log(
      `[KB] ${knownAnswers.length} known, ${unknownQuestions.length} unknown`
    );

    // Create help request for unknown questions
  ...
  // All questions known
  console.log("[KB] All questions answered from KB");
...
async function processSingleQuery(
  ...
  try {
    ...
    try {
      ...
      console.log("[KB] Vector search not available, using fallback");
    ...
    if (similarQuestions && similarQuestions.length > 0) {
      ...
      console.log(`[KB] Found ${similarQuestions.length} similar questions:`);
      similarQuestions.forEach((item, i) => {
        const similarity = item.similarity || 0;
        console.log(
          `  ${i + 1}. Similarity: ${similarity.toFixed(
            3
      ...
      if (topSimilarity < SIMILARITY_THRESHOLD) {
        console.log(
          `[KB] Top similarity ${topSimilarity.toFixed(
            3
          )} below threshold ${SIMILARITY_THRESHOLD} - escalating`
        );
        if (skipHelp) {
          return { found: false, needsHelp: true, answer: null };
        }
    ...
    if (!context || similarQuestions.length === 0) {
      console.log("[KB] No relevant context found - escalating");
      if (skipHelp) {
        return { found: false, needsHelp: true, answer: null };
      }
      return await handleUnknownQuery(query, participantId, roomName);
...
Your response (or "NEED_HELP" if the knowledge base doesn't directly answer this):`;
    ...
    if (needsHelp) {
      console.log("[KB] AI determined it needs help");
      if (skipHelp) {
        return { found: false, needsHelp: true, answer: null };
      }
      return await handleUnknownQuery(query, participantId, roomName);
...
async function handleUnknownQuery(query, participantId, roomName) {
  try {
    console.log("Storing unknown query as help request for supervisor");
...
export async function storeQA(question, answer, learnedFromRequestId = null) {
  try {
    ...
    console.log(`Stored new Q&A pair: "${question}"`);
    return true;
  } catch (error) {
